import { pgTable, text, serial, integer, boolean, timestamp, jsonb, varchar, date, decimal, foreignKey, uuid } from "drizzle-orm/pg-core";
...
export const emailAccounts = pgTable("email_accounts", {
  ...
  createdAt: timestamp("created_at").defaultNow(),
  userId: text("user_id"), // UUID string from Supabase users
  // SMTP configuration
...
export const userActivityLogs = pgTable("user_activity_logs", {
  ...
  metadata: jsonb("metadata"), // Optional JSON for additional details
...
// Changelog entries for tracking all system changes
export const changelogEntries = pgTable("changelog_entries", {
  id: uuid("id").defaultRandom().primaryKey(),
  timestamp: timestamp("timestamp").defaultNow().notNull(),
  userId: integer("user_id").references(() => users.id),
  changeType: text("change_type").notNull(), // 'smtp_update', 'role_change', 'tab_edit', etc.
  description: text("description").notNull(),
  payload: jsonb("payload"), // Store the actual change object
  ipAddress: text("ip_address"),
  userAgent: text("user_agent"),
});
...
export const pipelineCards = pgTable("pipeline_cards", {
  ...
  vertical: text("vertical").notNull(), // "brands", "agencies", "partners"
  currentStage: text("current_stage").notNull().default("1"), // ID of the pipeline stage
  userId: integer("user_id").references(() => users.id), // User who owns/created this card
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
