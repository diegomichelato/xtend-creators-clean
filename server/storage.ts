import { 
  ...
  changelogEntries, type ChangelogEntry, type InsertChangelogEntry
} from "@shared/schema";
import { db } from "./db";
...
export interface IStorage {
  ...
  createChangelogEntry(entry: { action: string; userId: string; metadata: any }): Promise<void>;
  ...
  // User-specific email account operations
  getUserEmailAccounts(userId: string): Promise<EmailAccount[]>;
  getCampaignsByEmailAccount(emailAccountId: number): Promise<Campaign[]>;
...
export class MemStorage implements IStorage {
  ...
  private userId = 4; // Updated to use existing user ID
  ...
  // Batch operations for faster uploads
  async createContactsBatch(contactsData: InsertContact[]): Promise<Contact[]> {
    const now = new Date();
    const createdContacts: Contact[] = [];
    
    for (const insertContact of contactsData) {
      const id = this.contactId++;
      const contact: Contact = { 
        ...insertContact, 
        id,
  ...
  async addContactsToListBatch(entries: { contactListId: number; contactId: number }[]): Promise<void> {
    for (const entry of entries) {
      const id = this.contactListEntryId++;
      const contactListEntry: ContactListEntry = {
        id,
        contactListId: entry.contactListId,
        contactId: entry.contactId,
        addedAt: new Date()
      };
      this.contactListEntries.set(id, contactListEntry);
    }
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async getAllUsers(): Promise<User[]> {
    try {
      ...
      console.log(`✅ Successfully fetched ${result.length} users from database:`, result.map(u => u.username));
      return result;
    } catch (error) {
      console.error('❌ Error getting users from database, falling back to memory:', error);
      const memoryUsers = this.memStorage.getAllUsers();
      console.log(`📦 Using ${(await memoryUsers).length} users from memory storage`);
      return memoryUsers;
    }
  }
  ...
  async createUser(userData: InsertUser): Promise<User> {
    ...
    try {
      ...
      console.error('❌ Database error creating user, using memory fallback:', error);
      const memUser = await this.memStorage.createUser(userData);
      console.log('📦 User created in memory:', memUser);
      return memUser;
    }
  }
  ...
  // Contact methods - using database storage
  ...
  async createContact(contactData: InsertContact) {
    try {
      ...
      return this.memStorage.createContact(contactData);
    }
  }
  
  async bulkCreateContacts(contactsData: InsertContact[]) {
    try {
      const createdContacts = await db.insert(contacts).values(contactsData).returning();
      return createdContacts;
    } catch (error) {
      console.error("Error bulk creating contacts in database:", error);
      return this.memStorage.bulkCreateContacts(contactsData);
    }
  }
  ...
  async archiveContact(id: number) {
    try {
      ...
      console.error("Error archiving contact in database:", error);
  ...
  async restoreContact(id: number) {
    try {
      ...
      console.error("Error restoring contact in database:", error);
      return this.memStorage.restoreContact(id);
    }
  }
  
  async getContactsByFilter(filterParams: ContactFilterParams) {
    try {
      // Build where conditions based on filter params
      const conditions = [];
      
      if (filterParams.industry) {
        conditions.push(eq(contacts.industry, filterParams.industry));
      }
      
      if (filterParams.type) {
        conditions.push(eq(contacts.type, filterParams.type));
      }
      
      if (filterParams.country) {
        conditions.push(eq(contacts.country, filterParams.country));
      }
      
      if (!filterParams.includeArchived) {
        conditions.push(sql`${contacts.archivedAt} IS NULL`);
      }
      
      const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
      
      return await db.select().from(contacts).where(whereClause).orderBy(desc(contacts.createdAt));
    } catch (error) {
      console.error("Error filtering contacts from database:", error);
      return this.memStorage.getContactsByFilter(filterParams);
    }
  }
  
  async getContactsByEmail(email: string) {
    try {
      return await db.select().from(contacts).where(eq(contacts.email, email));
    } catch (error) {
      console.error("Error fetching contacts by email from database:", error);
      return this.memStorage.getContactsByEmail(email);
    }
  }
  ...
  // Contact List methods - using database storage
  async getAllContactLists() {
    try {
      return await db.select().from(contactLists).orderBy(desc(contactLists.createdAt));
    } catch (error) {
      console.error("Error fetching contact lists from database:", error);
      return this.memStorage.getAllContactLists();
    }
  }
  
  async getContactList(id: number) {
    try {
      const [contactList] = await db.select().from(contactLists).where(eq(contactLists.id, id));
      return contactList;
    } catch (error) {
      console.error("Error fetching contact list from database:", error);
      return this.memStorage.getContactList(id);
    }
  }
  
  async createContactList(listData: InsertContactList) {
    try {
      const [contactList] = await db.insert(contactLists).values(listData).returning();
      return contactList;
    } catch (error) {
      console.error("Error creating contact list in database:", error);
      return this.memStorage.createContactList(listData);
    }
  }
  
  async updateContactList(id: number, listData: Partial<ContactList>) {
    try {
      const [contactList] = await db.update(contactLists).set(listData).where(eq(contactLists.id, id)).returning();
      return contactList;
    } catch (error) {
      console.error("Error updating contact list in database:", error);
      return this.memStorage.updateContactList(id, listData);
    }
  }
  
  async deleteContactList(id: number) {
    try {
      const [contactList] = await db.delete(contactLists).where(eq(contactLists.id, id)).returning();
      return contactList;
    } catch (error) {
      console.error("Error deleting contact list from database:", error);
      return this.memStorage.deleteContactList(id);
    }
  }
  
  // Contact List Membership methods - using database storage
  async getContactListMemberships(contactListId: number) {
    try {
      return await db.select().from(contactListEntries).where(eq(contactListEntries.contactListId, contactListId));
    } catch (error) {
      console.error("Error fetching contact list memberships from database:", error);
      return this.memStorage.getContactListMemberships(contactListId);
    }
  }
  
  async getContactMemberships(contactId: number) {
    try {
      return await db.select().from(contactListEntries).where(eq(contactListEntries.contactId, contactId));
    } catch (error) {
      console.error("Error fetching contact memberships from database:", error);
      return this.memStorage.getContactMemberships(contactId);
    }
  }
  
  async addContactToList(contactListId: number, contactId: number) {
    try {
      ...
      console.error("Error adding contact to list in database:", error);
      return this.memStorage.addContactToList(contactId, contactListId);
    }
  }
  
  async removeContactFromList(contactListId: number, contactId: number) {
    try {
      const [entry] = await db.delete(contactListEntries)
        .where(and(
          eq(contactListEntries.contactListId, contactListId),
          eq(contactListEntries.contactId, contactId)
        ))
        .returning();
      return entry;
    } catch (error) {
      console.error("Error removing contact from list in database:", error);
      return this.memStorage.removeContactFromList(contactId, contactListId);
    }
  }
  
  async bulkAddContactsToList(contactIds: number[], contactListId: number) {
    try {
      const entries = contactIds.map(contactId => ({
        contactListId,
        contactId
      }));
      const createdEntries = await db.insert(contactListEntries).values(entries).returning();
      return createdEntries;
    } catch (error) {
      console.error("Error bulk adding contacts to list in database:", error);
      return this.memStorage.bulkAddContactsToList(contactIds, contactListId);
    }
  }

  // Batch operations for faster contact uploads
  async createContactsBatch(contactsData: InsertContact[]): Promise<Contact[]> {
    try {
      const { db } = await import('./db');
      const { contacts } = await import('@shared/schema');
      
      console.log(`Starting batch insert of ${contactsData.length} contacts to database...`);
      
      // Insert all contacts in a single batch operation
      const insertedContacts = await db.insert(contacts).values(contactsData).returning();
      
      console.log(`Successfully batch inserted ${insertedContacts.length} contacts to database`);
      return insertedContacts;
    } catch (error) {
      console.error("Error in batch contact insert:", error);
      ...
      for (const contactData of contactsData) {
        try {
          ...
          console.error("Individual contact insert also failed:", individualError);
        }
      }
      return results;
    }
  ...
  async addContactsToListBatch(entries: { contactListId: number; contactId: number }[]): Promise<void> {
    try {
      const { db } = await import('./db');
      const { contactListEntries } = await import('@shared/schema');
      
      console.log(`Adding ${entries.length} contacts to list in batch...`);
      
      // Insert all contact list entries in a single batch operation
      await db.insert(contactListEntries).values(entries);
      
      console.log(`Successfully added ${entries.length} contacts to list`);
    } catch (error) {
      console.error("Error in batch contact list insert:", error);
      
      // Fallback to individual inserts
      for (const entry of entries) {
        try {
          await this.addContactToList(entry.contactListId, entry.contactId);
        } catch (individualError) {
          console.error("Individual contact list entry insert failed:", individualError);
        }
      }
    }
  }
  ...
  async createEmailAccount(accountData: InsertEmailAccount): Promise<EmailAccount> { 
    try {
      ...
      console.error('Error creating email account in database:', error);
      return this.memStorage.createEmailAccount(accountData);
    }
  }

  async updateEmailAccount(id: number, accountData: Partial<EmailAccount>): Promise<EmailAccount> { 
    try {
      const { db } = await import('./db');
      const { emailAccounts } = await import('@shared/schema');
      const { eq } = await import('drizzle-orm');
      ...
      return this.memStorage.updateEmailAccount(id, accountData);
    }
  }
  ...
  async deleteEmailAccount(id: number): Promise<boolean> { 
    try {
      ...
      console.error('Error deleting email account from database:', error);
      return this.memStorage.deleteEmailAccount(id);
    }
  }
  ...
  async getUserEmailAccounts(userId: string): Promise<EmailAccount[]> {
    try {
      ...
      console.error(`Error fetching email accounts for user ${userId}:`, error);
      return [];
    }
  }

  async getCampaignsByEmailAccount(emailAccountId: number): Promise<Campaign[]> {
    try {
      const { db } = await import('./db');
      const { campaigns } = await import('@shared/schema');
      const { eq } = await import('drizzle-orm');
      
      const accountCampaigns = await db.select().from(campaigns)
        .where(eq(campaigns.emailAccountId, emailAccountId));
      
      return accountCampaigns;
    } catch (error) {
      console.error(`Error fetching campaigns for email account ${emailAccountId}:`, error);
      return [];
    }
  }
  ...
  // Proposal methods - Using database storage
  async getAllProposals() { return this.getProposals(); }
  async getProposal(id: number): Promise<Proposal | undefined> {
    try {
  ...
  async updateProposal(id: number, proposalData: Partial<Proposal>): Promise<Proposal | undefined> {
    try {
      const [proposal] = await db.update(proposals).set(proposalData).where(eq(proposals.id, id)).returning();
      return proposal;
    } catch (error) {
  ...
  // Changelog operations
  async createChangelogEntry(entry: { action: string; userId: string; metadata: any }): Promise<void> {
    try {
      const { pool } = await import('./db');
      await pool.query(
        'INSERT INTO changelog (action, user_id, metadata) VALUES ($1, $2, $3)',
        [entry.action, entry.userId, JSON.stringify(entry.metadata)]
      );
    } catch (error) {
  ...
  async getUserActivityLogs(filters?: {
    ...
    try {
      ...
      if (filters?.startDate) {
        conditions.push(gte(userActivityLogs.timestamp, filters.startDate));
      }
      
      if (filters?.endDate) {
        conditions.push(lte(userActivityLogs.timestamp, filters.endDate));
      }
  ...
  async getActivityLogStats(filters?: {
    userId?: number;
    startDate?: Date;
    endDate?: Date;
  }): Promise<{
    ...
    try {
      ...
      const [statsResult] = await db.select({
        totalActions: count(),
        uniqueUsers: countDistinct(userActivityLogs.userId),
      })
      .from(userActivityLogs)
      .where(whereClause);
