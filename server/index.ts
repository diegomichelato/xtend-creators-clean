...
import { refreshDatabaseSchema, validateDatabaseSchema } from "./database-utils.js";
import { validateEmailConfig } from "./email-utils.js";
...
// Campaign routes are handled in the main routes file
  ...
  server.listen({
    ...
    try {
      ...
      if (!dbValidation.valid) {
        log(`Database schema issues detected: ${dbValidation.issues.join(', ')}`);
      } else {
      ...
      if (!emailValidation.valid) {
        log(`Email configuration issues: ${emailValidation.issues.join(', ')}`);
      } else {
        log(`Email configuration validated`);
      }
      
    } catch (initError) {
      log(`Production initialization error: ${initError instanceof Error ? initError.message : String(initError)}`);
    }
    
    // Start background jobs for Gmail token refresh and email sync
    try {
      const { backgroundJobs } = await import('./background-jobs');
      backgroundJobs.start();
      log(`Background jobs started successfully`);
    } catch (bgError) {
      log(`Background jobs startup failed: ${bgError instanceof Error ? bgError.message : String(bgError)}`);
    }
