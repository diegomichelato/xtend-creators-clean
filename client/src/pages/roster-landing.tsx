...
import { Instagram, Youtube, Music, Facebook, Star, Search, Filter, Users, Award, TrendingUp, Mail } from "lucide-react";
...
function getTopPlatform(creator: Creator) {
  ...
  return platforms.length > 0 ? platforms.sort((a, b) => b.count - a.count)[0] : null;
}

function parseFollowerCount(countStr: string): number {
  if (!countStr) return 0;
  const cleanStr = countStr.replace(/[^\d.KMB]/gi, '').toLowerCase();
  const num = parseFloat(cleanStr);
  
  if (cleanStr.includes('m') || cleanStr.includes('b')) return num * 1000000;
...
export default function RosterLanding() {
  ...
  const filteredCreators = creators?.filter(creator => {
    ...
    if (selectedTier && selectedTier !== "all") {
      const tier = getFollowerTier(creator);
  ...
  const totalFollowers = creators?.reduce((total, creator) => {
    const topPlatform = getTopPlatform(creator);
    return total + (topPlatform?.count || 0);
  }, 0) || 0;
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">Discover our talented content creators</p>
            </div>
            <div className="flex items-center gap-4 text-sm text-gray-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{totalFollowers >= 1000000 ? `${(totalFollowers / 1000000).toFixed(1)}M` : totalFollowers >= 1000 ? `${(totalFollowers / 1000).toFixed(1)}K` : totalFollowers.toLocaleString()} Total Reach</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold text-gray-900">
                    {creators?.filter(c => c.performance?.instagram?.followers).length || 0}
                  </div>
            ...
            <div className="bg-gradient-to-r from-red-50 to-red-100 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold text-gray-900">
                    {creators?.filter(c => c.performance?.youtube?.subscribers).length || 0}
                  </div>
            ...
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold text-gray-900">
                    {creators?.filter(c => c.performance?.tiktok?.followers).length || 0}
                  </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-bold text-gray-900">
                    {creators?.filter(c => c.performance?.facebook?.followers).length || 0}
                  </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-medium text-gray-700">Filter Creators</span>
          </div>
              ...
              <Card key={creator.id} className="group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-0 shadow-md">
                <CardContent className="p-6">
                  ...
                  <Button 
                    ...
                    size="sm"
                    onClick={() => window.open('mailto:partnerships@xtendcreators.com?subject=Partnership%20Inquiry%20-%20' + encodeURIComponent(creator.name), '_blank')}
                  >
        ...
        <div className="mt-16 text-center bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg p-8">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Ready to Partner With Us?</h2>
          <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
            Connect with our talented creators to amplify your brand's reach and engagement across all major platforms.
          </p>
          <Button 
            size="lg" 
            className="bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-white"
            onClick={() => window.open('mailto:partnerships@xtendcreators.com?subject=Partnership%20Inquiry', '_blank')}
          >
