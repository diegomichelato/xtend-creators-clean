...
const formSchema = z.object({
  ...
  userId: z.string().min(1, { message: "User assignment is required" }).refine(
    (val) => val !== "none", 
...
export function EmailAccountAddDialog({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      email: "",
      provider: "",
      userId: "", // This will be set when user selects from dropdown
      dailyLimit: 100,
      warmupEnabled: false,
    ...
    mutate: async (values: z.infer<typeof formSchema>) => {
      try {
        ...
        const transformedValues = {
          ...values,
          // Keep userId as string (it's a UUID, not a number)
        ...
        const result = await apiRequest("POST", "/api/email-accounts", accountData);
        ...
        if (result && (result.success || result.data)) {
          ...
          // Invalidate queries to refresh the email accounts list
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Every email account must be connected to a specific user - this field is required
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
