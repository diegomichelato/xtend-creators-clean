...
export function CSVUploader({ onUploadComplete, isLoading: externalIsLoading = false }: ContactUploaderProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      console.log("Starting upload with loading state:", true);
      await onUploadComplete(parsedData, listName);
      console.log("Upload completed successfully");
      
      // Show success message
      toast({
        title: "Upload successful!",
        description: `Successfully uploaded ${parsedData.length} contacts to ${listName}`,
      });
      ...
      // Add a small delay to ensure the upload completes properly for large files
      setTimeout(() => {
        setIsLoading(false);
        console.log("Loading state reset after upload");
      }, 1000);
    }
  };
